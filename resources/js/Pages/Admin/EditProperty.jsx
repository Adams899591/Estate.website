...
const AddProperty = () => {
       const {property,agents} = usePage().props;
      //  console.log(agents);
  ...
  function handleSubmit(e) {
    ...
    post(route('editProperty',{id : property.id}), {
    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Fill in the details to edit a property listing.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* <div> */}
              {/* <label htmlFor="admin_id" className="block text-sm font-medium text-slate-600 mb-1">Assign Agent</label> */}
              {/* <select id="admin_id" value={data.admin_id} onChange={e => setData('admin_id', e.target.value)} className="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm text-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500"> */}
                {/* this map true all the admin_id in the user table */}
               {/* {agents.map(agent => (
